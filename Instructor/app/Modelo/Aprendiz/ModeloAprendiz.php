...
class ModeloAprendiz extends conexion {
    ...
    #FUNCION PARA PAGINAR
     public function mostrarAprendizContadoModel() {
        ...
        $cmdsql = "SELECT count(*) FROM " . $tabla . " WHERE State = 'A' AND id_rol = 1";
        try {
    ...
    public static function mostrarLimitadoAprendizModel($q,$cant) {
        $tabla = 'persona';
        $cnx = conexion::singleton_conexion();  
        $select = "SELECT * FROM persona WHERE State = 'A' AND id_rol = 1 LIMIT ".$q.",".$cant;
        try {
    ...
    public function mostrarAprendizModel() {
        $tabla = 'persona';
        $enlace = conexion::singleton_conexion();
        $cmdsql = "SELECT * FROM " . $tabla . " WHERE State = 'A' AND id_rol = 1";
        try {
            $preparado = $enlace->preparar($cmdsql);
            // Especificamos el fetch mode antes de llamar a fetch()
            $preparado->setFetchMode(PDO::FETCH_ASSOC);
    ...
    public static function buscarAprendizModel($q) {
        $tabla = 'persona';
        $cnx = conexion::singleton_conexion();  
        $select = "SELECT * FROM persona WHERE State = 'A' AND id_rol = 1 AND 
