...
$(document).ready(function() {
    ...
    buttons: {
          ...
          buttons: [
...

                    {
                        ...
                        title:'Inventario SENA - Coordinaciones',
                        titleAttr: 'Copiar',
  ...
  function editarCoordinacion(id){
    swal({
            title: "Desea editar esta Coordinacion ?",
            icon: "info",
            buttons: true
        }).then((willDelete) => {
            if (willDelete) {
                try {
                    var at = id;
                    var postData = {'id_Coordinacion': at};
                    var dataString = JSON.stringify(postData);
                    $.ajax({
                        url: '../Vista/modulos/Coordinacion/editarCoordinacion.php',
  ...
  function eliminarCoordinacion(id){
    swal({
            title: "Desea eliminar el Coordinacion ?",
            icon: "info",
            buttons: true
        }).then((willDelete) => {
            if (willDelete) {
                try {
                    ...
                    var dataString = JSON.stringify(postData);
                    $.ajax({
                        url: '../Vista/modulos/Coordinacion/eliminarCoordinacion.php',
                        data: {detalleClases: dataString},
                        type: 'POST',
                        success: function (datos) {
                            if (resultado = "ok") {
                                 swal("Coordinacion Eliminado", {
                                 icon: "success",
                                });
                                    $('#Coordinacion' + id).remove();
                                } else {
                                    alert('No se ha eliminado el Coordinacion');
                                }
                        }
                    });
                } catch (e) {
